...
bool EdcEcc::isSectorOk(const u8* sector, SectorModeType sectorType)
{
    ...
    memcpy(oldEdc, sector + SectorModeOffsets[sectorType].edcOffset, EDC_SIZE);
    CalcEDC(sector, sectorType, newEdc);
    if(memcmp(oldEdc, newEdc, EDC_SIZE))
        return false;

    u8 oldEcc[ECC_SIZE], newEcc[ECC_SIZE];
    memcpy(oldEdc, sector + ECC_OFFSET, ECC_SIZE);
...
bool EdcEcc::fixSector(u8* sector, SectorModeType sectorType)
{
    ...
    CalcEDC(sector, sectorType, sector + SectorModeOffsets[sectorType].edcOffset);
    CalcECC(sector, sectorType, sector + SectorModeOffsets[sectorType].eccPOffset);
