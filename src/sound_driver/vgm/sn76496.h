...
void sn76496_w(uint8_t dd)
{
    ...
    if ((dd & 0x80) == 0)
    {
        if (chan < 2)
        {
            slots[chan].fns = sn_scsp_map[chip->period[chan]+0].fns;
            slots[chan].oct = sn_scsp_map[chip->period[chan]+0].oct;
            // slots[chan].oct = 5;
            ...
            // slots[chan].total_l = volume_map[chip->vol[chan] & 0xf];
